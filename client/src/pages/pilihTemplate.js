...
function PilihTemplate() {
  ...
  const groupedTemplates = templates.reduce((acc, template) => {
    const kategoriName = template.kategori?.nama_kategori || 'Kategori Tidak Diketahui';
    if (!acc[kategoriName]) {
      acc[kategoriName] = [];
    }
              ...
              <div className="space-y-6">
                {Object.keys(groupedTemplates).length === 0 ? (
                  <div className="text-center py-12">
                    ...
                    <h3 className="text-lg font-medium text-gray-900 mb-2">Tidak ada template surat</h3>
                    <p className="text-gray-500">Belum ada template surat yang tersedia.</p>
                  </div>
                ) : (
                  Object.entries(groupedTemplates).map(([kategoriName, templatesInKategori]) => (
                    <div key={kategoriName}>
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3">
                        {templatesInKategori.map((template) => (
                          <button
                            key={template.id}
                            onClick={() => navigate(`/buat-surat/template/${template.id}`)}
