...
function Validation() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const suratRes = await axios.get(`http://localhost:3001/suratKeluar/valid/${id_surat}`);
        setSurat(suratRes.data);
        // Ambil data kepala sekolah
        const kepalaRes = await axios.get('http://localhost:3001/user/kepala-sekolah');
  ...
  if (!surat || !kepalaSekolah) return <div className="p-8 text-center text-red-500">Data tidak ditemukan</div>;
  ...
  return (
    <div className="min-h-screen w-full flex flex-col items-center justify-center py-8 px-2 bg-gray-100">
      <div className="bg-white shadow-lg rounded-2xl px-4 py-8 sm:px-8 w-full max-w-sm md:max-w-lg text-center flex flex-col items-center">
        ...
        />
        {/* Teks Disetujui oleh */}
        <p className="text-gray-500 text-sm mb-2 tracking-wide">Disetujui oleh:</p>
        {/* Nama Kepala Sekolah */}
        <h2 className="text-lg md:text-xl font-bold mb-1 text-gray-800 tracking-wide">{kepalaSekolah.nama}</h2>
        {/* Tanda Tangan */}
        ...
        <div className="flex flex-col sm:flex-row justify-center items-center gap-2 mt-4 w-full">
          <div className="flex items-center gap-2 bg-gray-50 rounded-lg py-2 px-3 w-full sm:w-auto justify-center">
            ...
            <span className="text-base font-medium text-gray-700 break-all">{surat.nomor_surat || '-'}</span>
          </div>
          <div className="flex items-center gap-2 bg-gray-50 rounded-lg py-2 px-3 w-full sm:w-auto justify-center">
            <FiCalendar className="text-green-500 text-lg" />
            <span className="text-base font-medium text-gray-700">{formatDate(surat.tanggal_surat)}</span>
          </div>
        </div>
      </div>
    </div>
