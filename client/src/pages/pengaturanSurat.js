...
function PengaturanSurat() {
  ...
  const handleEditSubmit = async (e) => {
    ...
    try {
      ...
      await axios.put(`http://localhost:3001/kategori/${selectedKategori.kode_kategori}`, editFormData, {
        headers: {
          Authorization: `Bearer ${token}`
        }
      });
  ...
  const handleDelete = async (kode) => {
    if (window.confirm('Apakah Anda yakin ingin menghapus kategori surat ini?')) {
      try {
        const token = localStorage.getItem('token');
        if (!token) {
          navigate('/login');
      ...
      <div className="flex-1 p-4">
        ...
        <div className="bg-white shadow-sm rounded-lg p-5 mt-2">
          <div className="flex justify-between items-center mb-4">
            <h1 className="text-xl font-semibold text-gray-800">Kategori Surat</h1>
            <button
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {kategori.map((item) => (
                    <tr key={item.kategori_id}>
                      <td className="px-4 py-2 text-sm text-gray-900">{item.kode_kategori}</td>
                      <td className="px-4 py-2 text-sm text-gray-900">{item.nama_kategori}</td>
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-lg p-6 w-full max-w-md">
            <h2 className="text-xl font-semibold mb-4">Tambah Kategori Surat</h2>
            ...
            <form onSubmit={handleEditSubmit} className="space-y-4">
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setEditFormData({...editFormData, kode_kategori: e.target.value})}
                  placeholder="Masukkan kode kategori"
                />
              </div>
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setEditFormData({...editFormData, nama_kategori: e.target.value})}
                  placeholder="Masukkan nama kategori"
                />
              </div>
              <div>
