...
function PengesahanSurat() {
  ...
  const fetchSuratKeluar = async () => {
    try {
      ...
      const response = await axios.get('http://localhost:3001/suratKeluar/status/diproses', {
        headers: {
  ...
  const handlePengesahan = async (status) => {
    try {
      ...
      await axios.put(`http://localhost:3001/suratKeluar/${selectedSurat.id_surat}`, {
        status,
        catatan
      }, {
        headers: {
    ...
    <div className="flex min-h-screen bg-gray-50">
      ...
      <div className="flex-1 p-4">
        ...
        <div className="bg-white shadow-sm rounded-lg p-5 mt-2">
          <div className="mb-4">
            <h1 className="text-xl font-semibold text-gray-800">Pengesahan Surat Keluar</h1>
          </div>
            ...
            <div className="overflow-x-auto max-h-[70vh] overflow-y-auto">
              <table className="min-w-full divide-y divide-gray-200">
                <thead className="bg-gray-50">
                  <tr>
                    <th className="px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Tanggal Surat</th>
                    ...
                    <th className="px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Perihal</th>
                    <th className="px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Sifat</th>
                  ...
                  {suratKeluar.map((item) => (
                    <tr key={item.id_surat}>
                      ...
                      <td className="px-4 py-2">
                        <div className="text-sm font-medium text-gray-900">{item.pegawai?.nama || '-'}</div>
      ...
      {showModal && selectedSurat && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-lg max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            <div className="p-6">
              <h2 className="text-lg font-semibold text-gray-900 mb-4">Detail Surat Keluar</h2>
              <div className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-sm font-medium text-gray-700">Nomor Surat</label>
                    <div className="mt-1 text-sm text-gray-900">{selectedSurat.nomor_surat || '-'}</div>
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700">Tanggal</label>
                    <div className="mt-1 text-sm text-gray-900">{formatDate(selectedSurat.tanggal_surat)}</div>
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700">Kategori</label>
                    <div className="mt-1 text-sm text-gray-900">{selectedSurat.kategori?.nama_kategori || '-'}</div>
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700">Sifat</label>
                    <div className="mt-1 text-sm text-gray-900">{selectedSurat.sifat}</div>
                  </div>
                  <div className="col-span-1">
                    <label className="block text-sm font-medium text-gray-700">Perihal</label>
                    <div className="mt-1 text-sm text-gray-900">{selectedSurat.perihal}</div>
                  </div>
                  <div className="col-span-1">
                    <label className="block text-sm font-medium text-gray-700">Tujuan</label>
                    <div className="mt-1 text-sm text-gray-900">{selectedSurat.tujuan}</div>
                  </div>
                  {selectedSurat.kepada && (
                    <div className="col-span-2">
                      <label className="block text-sm font-medium text-gray-700">Kepada</label>
                      <div className="mt-1 text-sm text-gray-900">{selectedSurat.kepada}</div>
                    </div>
                  )}
                  <div className="col-span-2">
              ...
              <div className="mt-6 flex justify-end gap-2">
                ...
                <button
                  onClick={() => {
                    if (window.confirm('Apakah Anda yakin ingin menyetujui surat ini? Tindakan ini akan men-generate QR code dan tidak bisa dibatalkan.')) {
                      handlePengesahan('disetujui');
                    }
                  }}
                  className="px-4 py-2 text-sm font-medium text-green-600 hover:text-green-700 hover:bg-green-50 border border-gray-300 rounded-md transition-colors"
                >
                  Setujui
                </button>
                <button
                  onClick={() => {
                    if (window.confirm('Apakah Anda yakin ingin menolak surat ini?')) {
                      handlePengesahan('ditolak');
                    }
                  }}
                  className="px-4 py-2 text-sm font-medium text-red-600 hover:text-red-700 hover:bg-red-50 border border-gray-300 rounded-md transition-colors"
                >
                  Tolak
                </button>
              </div>
