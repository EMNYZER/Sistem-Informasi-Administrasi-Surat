...
router.post('/upload-lampiran/:id_surat', authenticateToken, uploadLampiran.single('file'), async (req, res) => {
  try {
    ...
    console.error(error);
    res.status(500).json({ error: 'Gagal upload lampiran', detail: error.message });
  }
});
...
router.get('/', authenticateToken, async (req, res) => {
  try {
    ...
    res.status(500).json({ error: 'Terjadi kesalahan saat mengambil data surat keluar', message: error.message });
  }
});
...
router.get('/:id_surat', authenticateToken, async (req, res) => {
  try {
    ...
    if (!surat) return res.status(404).json({ error: 'Surat keluar tidak ditemukan' });
    res.json(surat);
  } catch (error) {
    res.status(500).json({ error: 'Terjadi kesalahan saat mengambil data surat keluar', message: error.message });
  }
...
router.post('/', authenticateToken, async (req, res) => {
  try {
    ...
    res.status(500).json({ error: 'Terjadi kesalahan saat menambah surat keluar', message: error.message });
  }
});
...
router.put('/:id_surat', authenticateToken, async (req, res) => {
  try {
    ...
    const { status, lampiran_file } = req.body;
    ...
    // === Hapus lampiran lama jika akan diganti ===
    if (req.body.ganti_lampiran === 'true' && surat.lampiran_file) {
      const oldFilename = path.basename(surat.lampiran_file);
      const oldPath = path.join(__dirname, '..', 'public', 'uploads', 'lampiran', oldFilename);

      if (fs.existsSync(oldPath)) {
        fs.unlinkSync(oldPath);
      }

      // Kosongkan dulu lampiran_file agar nanti diisi ulang setelah upload
      surat.lampiran_file = null;
    }

    // === Update field lainnya (termasuk "lampiran") ===
...
router.delete('/:id_surat', authenticateToken, async (req, res) => {
  try {
    ...
    if (surat.lampiran) {
      ...
      const lampiranFilePath = path.join(__dirname, '..', 'public', 'uploads', 'lampiran', lampiranFilename);
    ...
    res.json({ message: 'Surat keluar, QR code, dan lampiran berhasil dihapus' });
  } catch (error) {
    console.error('Gagal hapus surat:', error);
    res.status(500).json({ error: 'Terjadi kesalahan saat menghapus surat keluar', message: error.message });
  }
});
...
router.post('/generate-nomor/:id_surat', authenticateToken, async (req, res) => {
  try {
    ...
    // Update surat
    await SuratKeluar.update({ nomor_urut_surat: nomor_urut, nomor_surat }, { where: { id_surat } });
    res.json({ nomor_surat });
  } catch (error) {
    res.status(500).json({ error: 'Gagal generate nomor surat', message: error.message });
  }
});
