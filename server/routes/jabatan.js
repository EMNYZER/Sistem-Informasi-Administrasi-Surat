...
router.put('/:jabatan_id', authenticateToken, async (req, res) => {
     try {
          const jabatan = await Jabatan.update(req.body, { where: { jabatan_id: req.params.jabatan_id } });
          res.json(jabatan);
     } catch (error) {
          res.status(500).json({ 
...
router.delete('/:jabatan_id', authenticateToken, async (req, res) => {
     try {
          const jabatan = await Jabatan.destroy({ where: { jabatan_id: req.params.jabatan_id } });
          res.json(jabatan);
     } catch (error) {
          res.status(500).json({ 
