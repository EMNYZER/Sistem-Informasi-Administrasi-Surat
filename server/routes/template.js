...
router.get('/', authenticateToken, async (req, res) => {
  try {
    ...
    res.status(500).json({ error: 'Terjadi kesalahan saat mengambil data template', message: error.message });
  }
});
...
router.post('/', authenticateToken, async (req, res) => {
  try {
    ...
    res.status(500).json({ error: 'Terjadi kesalahan saat menambah template', message: error.message });
  }
});
...
router.put('/:id', authenticateToken, async (req, res) => {
  try {
    ...
    res.status(500).json({ error: 'Terjadi kesalahan saat mengupdate template', message: error.message });
  }
});
...
router.delete('/:id', authenticateToken, async (req, res) => {
  try {
    ...
    if (!deleted) return res.status(404).json({ error: 'Template tidak ditemukan' });
    res.json({ message: 'Template berhasil dihapus' });
  } catch (error) {
    res.status(500).json({ error: 'Terjadi kesalahan saat menghapus template', message: error.message });
  }
});
